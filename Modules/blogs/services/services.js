...
const createBlogSer = async (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const { heading, paragraph, blogImage, createdBy, description } = req.body;
...
const availableBlogSer = async (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const availableBlog = await blogsDB
        .find({})
        .sort({ _id: -1 })
      ...
      return resolve({
        status: 200,
        count: availableBlog.length,
        message: "successfully fetched available Blog",
        totalDocuments: totalDocs,
        totalPages: totalPages,
        data: {
          availableBlog: availableBlog,
        },
      });
    } catch (error) {
      return reject({
    ...
    try {
      ...
      if (!existPlan) {
        ...
        const updatedObj = {
          ...
          blogImage: blogImage || existPlan.blogImage,
        };
        const updatedBlog = await blogsDB.findOneAndUpdate(
          { _id: _id },
          { $set: updatedObj },
          { new: true }
        );
